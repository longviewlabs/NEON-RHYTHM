...
export const useVideoRecorder = (
    ...
    // Target FPS for frame sending (lower = better perf, 20-24 is enough for social video)
    const TARGET_FPS = IS_MOBILE ? 20 : 24;
    ...
    useEffect(() => {
        ...
        canvas.width = IS_MOBILE ? 360 : 720;
        canvas.height = IS_MOBILE ? 640 : 1280;
    ...
    const startRecording = useCallback(() => {
        ...
        if (!videoRef.current || !canvasRef.current || !workerRef.current || !isWorkerReadyRef.current) {
        ...
        try {
            const recorder = new MediaRecorder(stream, {
                mimeType,
                videoBitsPerSecond: IS_MOBILE ? 800000 : 2500000,
            });
