...
const SequenceDisplay: React.FC<SequenceDisplayProps> = ({
  ...
  const currentScale = sequence.length > 20 ? "scale-[1.3]" : "scale-[1.5]";

  return (
    <div className="flex flex-wrap justify-center items-center select-none w-full max-w-4xl mx-auto px-4 gap-y-2 md:gap-y-6 transition-all duration-500 opacity-100">
      {sequence.map((num, i) => {
        ...
        if (result === false) {
          ...
          // CURRENT: Yellow
          displayClass += ` text-yellow-400 ${currentScale} drop-shadow-[0_0_30px_rgba(250,204,21,0.6)] z-10 font-black`;
        } else {
        ...
        return (
          <div
            key={i}
            className={`flex items-center whitespace-nowrap font-bold ${fontSizeClass} text-white drop-shadow-[0_2px_2px_rgba(0,0,0,1)]`}
          >
